...
DATA_SCIENCE_IMAGE = (
    "quay.io/modh/runtime-images:runtime-datascience-ubi9-python-3.9-2024a-20241011"  # noqa
)
...
def train_model(texts: list, labels: list) -> float:
    ...
    pipeline = make_pipeline(
        TfidfVectorizer(),
        MultinomialNB(),  # Converts text into TF-IDF features
    )
...
def verify_accuracy(accuracy: float, threshold: float):
    ...
    if accuracy >= threshold:
        ...
        print(
            f"The model did not achieve the minimum accuracy of {threshold * 100:.2f}%."
        )
        print(f"Accuracy: {accuracy * 100:.2f}%")
        sys.exit(1)
...
def pipeline():
    ...
    verify_accuracy(accuracy=accuracy, threshold=0.9)
...
if __name__ == "__main__":
    ...
    print(f"Pipeline compiled.\nUse the RHOAI dashboard to import the '{outfile}' file")
